<template>
  <div class="user-management">
    ...
    <div class="card">
      <div class="card-head"><div class="card-title">用户列表</div><div class="card-action">邀请用户</div></div>
      <el-table :data="users" style="width: 100%" v-loading="loading">
        <el-table-column label="姓名" min-width="160">
          <template #default="{ row }"><div class="user-name-cell"><div class="user-avatar" :style="{ background: getAvatarBg(row.name), color: getAvatarColor(row.name) }">{{ row.name.charAt(0) }}</div><span>{{ row.name }}</span></div></template>
        </el-table-column>
    ...
    <el-dialog v-model="editVisible" title="编辑用户" width="520px" destroy-on-close>
      <el-form :model="editForm" label-width="80px">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0 16px">
          ...
          <el-form-item label="所属项目"><el-input v-model="editForm.project" /></el-form-item>
        </div>
        <el-form-item label="状态">
          <el-select v-model="editForm.status" style="width: 100%">
            <el-option label="活跃" value="活跃" /><el-option label="离线" value="离线" /><el-option label="待激活" value="待激活" /><el-option label="禁用" value="禁用" />
          </el-select>
        </el-form-item>
      </el-form>
...
import { getUsers, getUserStats, updateUser, deleteUser } from '../../api/user'
...
onMounted(async () => {
  ...
  loading.value = true
  try {
    const [usersRes, statsRes, rolesRes] = await Promise.all([getUsers(), getUserStats(), getRoles()])
