...
| 知识库 | Sprint 知识快照 → 模块 → 文档 + AI 功能点提取，支持文件上传和文档预览 |
...
- [x] 后端知识库 API（KnowledgeBase/Folder/Document CRUD + 文件上传）
- [x] Docker Compose 部署（前端 + 后端 + PostgreSQL）
- [x] 部署脚本（`deploy.ps1` 自动 Git push + 服务器 Docker 重建）

### 进行中（参见 全栈开发方案.md）
- [ ] 知识库模型重构：KnowledgeBase/Folder → Sprint/Module/Document
- [ ] TestCase 字段补充（前置条件/测试步骤/预期结果）
- [ ] Report 字段补充（报告类型/审批流程）
- [ ] FeaturePoint 功能点模型
- [ ] AI 配置后端（Provider/Strategy/Config/CallLog）
- [ ] 知识图谱后端（Graph/Node/Edge）
- [ ] AI 流水线后端（PipelineExecution/Stage）
- [ ] 全局搜索
